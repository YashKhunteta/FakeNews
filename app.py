...
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', '4c99e0361905b9f941f17729187afdb9')

# Load the dataset once to improve efficiency and handle missing dataset gracefully
...
def random():
    """
    Endpoint to fetch a random entry from the dataset.
    """
    if data is not None:
        index = randrange(0, len(data) - 1, 1)
        return jsonify({'title': data.loc[index].title, 
